...
* Find a matrix $\symbf{A}$ such that $\symbf{\Sigma} = \symbf{A}\,\symbf{A}^t$, most often we use $\symbf{A} = \text{Chol}(\symbf{\Sigma})$
...
library(mvtnorm)
rbind(
  cbind(param="rho=0.9",  rmvnorm(1000, sigma = matrix(c(1,0.9,0.9,1),2))   %>% as.data.frame() %>% setNames(c("x","y")) ),
  cbind(param="rho=0.7",  rmvnorm(1000, sigma = matrix(c(1,0.7,0.7,1),2))   %>% as.data.frame() %>% setNames(c("x","y")) ),
  cbind(param="rho=0.5",  rmvnorm(1000, sigma = matrix(c(1,0.5,0.5,1),2))   %>% as.data.frame() %>% setNames(c("x","y")) ),
  cbind(param="rho=0.1",  rmvnorm(1000, sigma = matrix(c(1,0.1,0.1,1),2))   %>% as.data.frame() %>% setNames(c("x","y")) ),
  cbind(param="rho=-0.9", rmvnorm(1000, sigma = matrix(c(1,-0.9,-0.9,1),2)) %>% as.data.frame() %>% setNames(c("x","y")) ),
  cbind(param="rho=-0.7", rmvnorm(1000, sigma = matrix(c(1,-0.7,-0.7,1),2)) %>% as.data.frame() %>% setNames(c("x","y")) ),
  cbind(param="rho=-0.5", rmvnorm(1000, sigma = matrix(c(1,-0.5,-0.5,1),2)) %>% as.data.frame() %>% setNames(c("x","y")) ),
  cbind(param="rho=-0.1", rmvnorm(1000, sigma = matrix(c(1,-0.1,-0.1,1),2)) %>% as.data.frame() %>% setNames(c("x","y")) )
...
\textbf{Proposition} - For an $n$-dimensional multivate normal with mean $\symbf{\mu}$ and covariance matrix $\symbf{\Sigma}$, any marginal or conditional distribution of the $y$'s will also (multivariate) normal.
...
* We will only have a (small) finite number of observations $Y(t_1), \ldots, Y(t_n)$ with which to say something useful about this infinite dimension process.
...
d_pred_s = rbind(
  cbind(
    data_frame(cov="Post Mean Model - sigma2=2.18, l=7.21"),
    d_pred,
    cond_pred(d_pred, d, cov = sq_exp_cov, l = 7.21, sigma2 = 2.18, reps=1000) %>% t() %>% post_summary()
  ),
  cbind(
    data_frame(cov="Post Median Model - sigma2=1.89, l=5.86"),
    d_pred,
    cond_pred(d_pred, d, cov = sq_exp_cov, l = 6.79, sigma2 = 1.86, reps=1000) %>% t() %>% post_summary()
  )
)
