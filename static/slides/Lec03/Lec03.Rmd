...
rjags::coda.samples(
  ...
  n.iter=500000, thin=100, progress.bar="none"
) %>%
  tidybayes::gather_samples(beta[i]) %>%
  mutate(param = paste0(term,"[",i,"]")) %>%
  ggplot(aes(x=.iteration*100, y=estimate)) + 
    geom_line() + 
    facet_wrap(~param, ncol=1, scale="free_y") +
    labs(x="Iteration")
```
...
tidybayes::spread_samples(samp3, Y_hat[i], lambda[i], X[i], Y[i]) %>%
  ungroup() %>%
  tidyr::gather(param, value, Y_hat, lambda) %>%
  ggplot(aes(x=X,y=Y)) +
    tidybayes::stat_lineribbon(aes(y=value), alpha=0.5) +
...
```{r echo=FALSE, fig.height=4}
tmp = aids %>% select(X=year, standard2:deviance2) %>%
  tidyr::gather(resid, value, standard2:deviance2) %>%
  mutate(resid = forcats::as_factor(resid))

tidybayes::spread_samples(samp3, lambda[i], X[i], Y[i]) %>%
  ungroup() %>%
  mutate(
    standard2 = (Y-lambda),
    pearson2  = (Y-lambda) / sqrt(lambda),
    deviance2 = dev_resid(Y, lambda)
  ) %>%
  tidyr::gather(resid, value, standard2:deviance2) %>%
  mutate(resid = forcats::as_factor(resid)) %>%
  ggplot(aes(x=as.factor(X),y=value,color=resid)) +
